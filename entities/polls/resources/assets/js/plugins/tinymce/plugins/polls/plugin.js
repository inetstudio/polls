window.tinymce.PluginManager.add('polls', function (editor) {
        ...
        widget: {
            events: {
                widgetSaved: function (model) {
                    editor.execCommand('mceReplaceContent', false, '<img class="content-widget" data-type="poll" data-id="'+model.id+'" alt="Виджет-опрос" />');
                }
            }
        }
    };
    ...
    function initPollsComponents() {
        if (typeof window.Admin.vue.modulesComponents.$refs['polls_PollWidget'] == 'undefined') {
            window.Admin.vue.modulesComponents.modules.polls.components = _.union(window.Admin.vue.modulesComponents.modules.polls.components, [
                {
                    name: 'PollWidget',
                    data: widgetData
                }
